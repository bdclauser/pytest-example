"""
Simple class modeling a cucumber basket.
Example taken from AndyLPK247/tau-pytest-bdd/chapter-3/cucumbers.py
...
class CucumberBasket:
    def __init__(self, initial_count=0, max_count=10):
        if initial_count < 0:
            raise ValueError("Initial cucumber basket count must not be a negative")
        if max_count < 0:
            raise ValueError("Max cucumber basket count must not be a negative")
